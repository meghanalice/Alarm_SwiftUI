            ...
            VStack {
                ...
                HStack(
                    ...
                    HStack(alignment: .lastTextBaseline) {
                        ...
                        Text(
                            "\(Image(systemName: icon.rawValue)) \(title.isEmpty ? "Timer" : title)"
                        )
                        .font(.system(size: 16))
                        .layoutPriority(3)
            ...
            case .countdown(let countdown):
                ...
                ProgressView(
                    timerInterval: countdown
                        .startDate...countdown.startDate.addingTimeInterval(remaining),
                    countsDown: true,
                    label: {},
                    currentValueLabel: {}
                )
    ...
    var body: some View {
        ...
        HStack(spacing: 8) {
            ...
            case .paused(_):
                ...
                Button(
                    intent: ResumeIntent(alarmID: id),
                    label: {
...
#Preview(
    "Dynamic Compact", as: .dynamicIsland(.compact),
    using: AlarmAttributes<_AlarmMetadata>.testAttributes
) {
    CountdownLiveActivity()
} contentStates: {
    AlarmAttributes<_AlarmMetadata>.ContentState.testState
...
#Preview(
    "Dynamic Minimal", as: .dynamicIsland(.minimal),
    using: AlarmAttributes<_AlarmMetadata>.testAttributes
) {
    CountdownLiveActivity()
} contentStates: {
...
#Preview(
    "Dynamic Expanded", as: .dynamicIsland(.expanded),
    using: AlarmAttributes<_AlarmMetadata>.testAttributes
) {
    CountdownLiveActivity()
} contentStates: {
...
extension AlarmAttributes<_AlarmMetadata>.ContentState {
    fileprivate static var testState: AlarmAttributes<_AlarmMetadata>.ContentState {
        .init(
            alarmID: UUID(),
            mode: .countdown(
                .init(
                    totalCountdownDuration: 60 * 60 * 24, previouslyElapsedDuration: 0,
                    startDate: Date(), fireDate: Date())))
    }
}

extension AlarmAttributes<_AlarmMetadata> {
    fileprivate static var testAttributes: Self {
        let title = "title"
